...
public class GameIntro extends JFrame implements GLEventListener {
	...
	public void display(GLAutoDrawable draw) {
		...
		gl.glColor3d(3d, 3d, 1d);

		gl.glClear(GL2.GL_COLOR_BUFFER_BIT | GL2.GL_DEPTH_BUFFER_BIT);
		gl.glLoadIdentity();// Reset The View

		gl.glRotated(rpoly, -0.1d, -2 / 7.4 * Math.PI, -0.2d);
		gl.glTranslated(0.1d, -2.7d, 0.1d);
		...
		glut.glutWireSphere(30, 2, 2400);

		gl.glEnd();

		gl.glFlush();

		rpoly -= -1.375; // assigning the angle
	...
	public void reshape(GLAutoDrawable draw, int y, int x, int width, int height) {
		...
		glu.gluLookAt(0, 500, 500, 0, 0, 0.1, 0, 1, 0);
