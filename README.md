...
For the MySQL in LoadDriver.java, I use Amazon Web Services RDS MySQL Database and an EC2 instance for Security Groups, to allow incoming and outgoing TCP connections to "Your IP Address(which can change periodically)" through port 3306. My website was registered on dot.tk or freedomains.com and you simply take the NS Records from your DNS management options in dot.tk and put them into an AWS Hosted Zone, allowing all connections. MySQL only allows connections through port 3306 so you can use the same EC2 instance and Security group. Your HTML, CSS etc. can be stored in S3 and I will eventually be looking into DevOps for JSP, and Elastic Beanstalk to host an admin version of my account. Spring Framework also may have their own cloud if you want to look into that route or many others such as Google Cloud, Heliohost etc. 
